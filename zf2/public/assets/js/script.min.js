...
(change_song(b),!1!=a.update_mod&&change_mod(a.update_mod));-1!=a.localip.indexOf(dj_ip)&&change_song(b);"refresh_this_123"==b&&(location.href=location.href)});Server.connect()});function send(a){log("You:"+a);Server.send("message",JSON.stringify({message:a,localip:localip}))}function log(a){$log=$("#log");$log.append(($log.val()?"\n":"")+a);$log[0].scrollTop=$log[0].scrollHeight-$log[0].clientHeight}Pusher.log=function(a){window.console&&window.console.log&&window.console.log(a)};
var pusher=new Pusher(pusher_config.key),channel=pusher.subscribe(pusher_config.channel);
