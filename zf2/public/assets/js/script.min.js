...
$(document).ready(function(){log("Connecting...");Server=new FancyWebSocket(server_name);Server.bind("open",function(){log("Connected.")});Server.bind("close",function(a){log("Disconnected.")});Server.bind("message",function(a){a=JSON.parse(a);var b=a.message;"undefined"===typeof a.updatedj&&(a.updatedj=!1);"undefined"===typeof a.localip&&(a.localip="wqeqwe");console.log(b+"from"+a.localip);!1!=a.updatedj&&(dj_ip=a.updatedj);-1!=a.localip.indexOf(dj_ip)&&change_song(b);"refresh_this_123"==b&&(location.href=
location.href)});Server.connect()});function send(a){log("You:"+a);Server.send("message",JSON.stringify({message:a,localip:localip}));$.post("/ajax/send/",{message:a,localip:localip})}function log(a){console.log(a)}Pusher.log=function(a){window.console&&window.console.log&&window.console.log(a)};var pusher=new Pusher(pusher_config.key),channel=pusher.subscribe(pusher_config.channel);
...
$(document).ready(function(){$(document).on("click",".btbtn",function(){toggleMute(this)});$(document).on("click","#update_mod",function(){$.post("/ajax/setMessage/",{update_mod:$(".message").val(),localip:localip})})});function getVolume(){var a=0;$("audio,video").each(function(){a=$(this).context.volume});return a}function setVolume(a){$("audio,video").each(function(){$(this).context.volume=a})}
